...
def update_correlation_viz(n_clicks, viz_type, year_range):
    ...
    # Always start with a blank figure - critical fix!
    fig = go.Figure()
    
    # Create a completely new figure for each visualization type
    if viz_type == "time":
        # Create time series with secondary y-axis
        ...
        # Create heatmap - explicitly starting with a new figure
        fig = go.Figure()
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                texttemplate="%{text}",
                hovertemplate="Variables: %{x} & %{y}<br>Correlation: %{z:.3f}<extra></extra>"
            )
        )
        
        # Update layout specifically for correlation matrix
        fig.update_layout(
        ...
        # Create scatter matrix - completely new approach
        fig = go.Figure()
        ...
        fig = px.scatter_matrix(
            ...
            # Hide the diagonal histograms if you prefer just the off-diagonals
    ...
    # Apply layout settings appropriate to ALL visualizations
    fig.update_layout(
        # Force clean slate with uirevision
        uirevision=viz_type,  # Critical for clean switching between viz types
    ...
    # Return completely new figure object
